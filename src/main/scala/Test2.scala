...
object Test2 {
	def main(args: Array[String]) {
		case class fileClass(x1: Double, x2: Double, x3:Double,x4:Double,x5:Double,x6:Double,x7:Double)
	...
	def getNNeighbors(fileName:String,N:Int,sc:SparkContext):RDD[(Long, Array[(Long, Double)])]={
		...
		val neighbors = annModel.neighbors(N)
		neighbors
	
	}
	def getReachDistance(neighbors:RDD[(Long, Array[(Long, Double)])],minPoints:Integer):RDD[(Long, Array[ Double])]= {
		val kDistance =0.02
	...
	def getLOF(reachDist:RDD[(Long, Double)],neighbors:RDD[(Long, Array[(Long, Double)])]):RDD[(Long,Double)]={
		...
		val something = neighborWithReach.flatMapValues(x=>x)
		val somethingNew= something.map(values=>(values._2._1,values._1))
		val result=somethingNew.join(reachDist).map(values=>(values._2._1._1,(values._2._1._2,values._2._2)))
